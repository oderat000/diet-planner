 ...
 *
 * Used only for the optional AI add-ons — dish recognition and the Q&A assistant. It only
 * runs server-side, inside the analyze-dish/ask API routes deployed on Vercel, so the key
 * never reaches the browser. The static (GitHub Pages) build of the app calls those routes
 * over HTTPS instead of importing this module. The core diet planner never touches this;
 * it stays fully offline. If no key is set, `hasGeminiKey()` is false and the routes disable
 * those features cleanly rather than error.
 *
